...
tokenizeDoc :: (TVar String, String) -> IO (String, [String])
tokenizeDoc (thread_print, filename) = withPdfFile filename $ \pdf -> do
    ...
    case result of 
        ...
        Right catalog -> do
            ...
            -- Finishing thread
            atomWrite thread_print ("Reading " ++ filename ++ " (\n" ++ tokenizePages_print ++ ");")
...
get_doc_data :: [String] -> TVar DocumentData -> IO (String, [String]) -> IO ()
get_doc_data tokens tvar io_data = do
    data_ <- io_data
    let content = snd data_
    let token_freq_ = zip tokens (map (token_frequency content) tokens)
    let len = length content

    (len, token_freq_) `deepseq` (atomWrite tvar (DocumentData {
